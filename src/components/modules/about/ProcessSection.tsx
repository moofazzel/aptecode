...
/** ---- Timing control (mirrors About section) ---- */
const BASE = 0.15; // first thing to appear
const STEP = 0.12; // gap between siblings

export default function ProcessSection() {
  return (
    <section className="relative pb-16 lg:pb-[96px] md:py-18">
      ...
      <div className="mx-auto max-w-[1405px] px-4 md:px-6">
        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:mt-16 md:grid-cols-3 sm:grid-cols-2">
          {STEPS.map((step, idx) => (
            <MotionCard
              key={step.id}
              delay={BASE + STEP * (2 + idx)} // starts after title, then staggers per card
              direction="up"
            >
              <article className="bg-[#f2f3f4] p-8 md:p-10 shadow-[0_1px_0_0_rgba(0,0,0,0.02)]">
                {/* Number pill */}
                ...
                {/* Description */}
                <p className="mt-3 text-[#74787c] leading-relaxed">
                  {step.desc}
                </p>
